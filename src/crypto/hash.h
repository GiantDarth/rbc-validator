...
/// A generic function that allows for any combination of hash functions through OpenSSL's EVP
/// system. \param digest The output digest. It must be pre-allocated with at least the correct
/// digest size. \param digest_size Either NULL for any non-XOF, or the wanted digest size when
/// using EVP_shake128 or EVP_shake256. Using a non-NULL size on a non-XOF is undefined behavior.
/// \param ctx A context the perform the digest operations in. If NULL (aka. not pre-allocated),
/// then it will allocate and destroy a temporary context at the cost of performance. \param md
/// Which hash function to use. \param msg The message to hash, must have at least msg_size bytes
/// allocated. \param msg_size How many bytes are allocated for msg. \param salt An additional,
/// optional salt that will perform a secondary update after msg. \param salt_size How big the salt
/// is. If salt is NULL, then this value is ignored. \return Returns a 0 on success and a 1 on
/// failure.
